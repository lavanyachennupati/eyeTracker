...
function runExperiment(pathName, orderFilePath, operationMode, subjectNumber,... 
  ...
  %Check to make sure that the user did not try to use eye tracking mode with 
  %movies
  if(operationMode == movieMode && eyeTrackingMode == 1)
    fprintf('This is not a valid mode for this phase. Please check phase 3.\n');
    return
  end
